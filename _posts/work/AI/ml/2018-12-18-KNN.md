...
样本库中每一个样本都有标签，查询样本的类别就由距离他最近的 K 个样本投票决定（当然存在特殊情况，就是某个类别的库存样本数量小于 K，这种情况下自行决定只用当前样本进行投票，还是施加权重；当然实际应用中很少会遇到这种情况）；    
这个策略常被用于**检索**；    
投票的具体过程：    
- 计算查询样本距离库中所有样本的距离；    
- 标签相同的前 K 个样本（需排序）进行投票（取和或者均值）；    
- （此时已经得到查询样本距离所有类别的距离）取最小距离即可；    
（可以看代码）     

<span id="knn_regression">**4. KNN 回归**</span>  
输出是该对象的属性值；该值是其 k 个最近邻居的值的平均值；  
`此处感觉和分类很相似啊；`{:.warning}    
据猜测，是库存样本在持续更新，使得每一次计算的出的 KNN 结果都不同，再根据此结果指导训练，得到下一次更新；往复迭代，所以有回归一说；      
