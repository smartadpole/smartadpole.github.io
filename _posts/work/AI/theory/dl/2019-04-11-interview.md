...
| [卷积操作作用](#convolution) | [卷积与全连接的区别](#conv_dense) | [pooling 层作用](#pooling) | [特征图大小计算](#feature_map_size) | [感受野及其计算](#receptive) |
| [BN 原理及作用](#bn) |  |  |  |  |
|  |  |  |  |  |
|  |  |  |  |  |
...
| 进阶 |  |  |  |
| --- | --- | --- | --- |
| [交叉熵为什么能用做 loss](#cross_entropy_loss) |  |  |  |
| [权值共享](#weight_share) |  |  |  |
| [优化算法](#optimizer) |  |  |  |
|  |  |  |  |
|  |  |  |  |
...
1. **加速训练过程**：      
    - **可以用更高的学习率**：不同层，不同维度上权重的范围都不一样，这就需要不同的学习率；为了避免梯度爆炸并保证有效地学习，会选用最小的学习率；而使用 BN 后，每个维度的权重值的范围都在一致的区间内，那么就可以使用较高的学习率；     
    - **移除或使用较低的 dropout**：dropout 是防止过拟合的，使用 BN 后过拟合现象会大大缓解，就可以少用或者去掉 dropout，这就加速了学习过程；    
1. 降低L2权重衰减系数：还是一样的问题，边界处的局部最优往往有几维的权重（斜率）较大，使用L2衰减可以缓解这一问题，现在用了 Batch Normalization，就可以把这个值降低了，论文中降低为原来的5倍；    
1. 取消 Local Response Normalization层：由于使用了一种Normalization，再使用LRN就显得没那么必要了；而且 LRN 实际上也没那么 work；
1. **Batch Normalization调整了数据的分布，不考虑激活函数，它让每一层的输出归一化到了均值为0方差为1的分布**，这保证了梯度的有效性，可以解决反向传播过程中的梯度问题。目前大部分资料都这样解释，比如BN的原始论文认为的缓解了Internal Covariate Shift(ICS)问题；  
...
神经网络训练是使用反向传播改变网络的参数，以降低误差；该过程使用了链式求导法则，将最后一层损失逐层传递，直到网络的第一层；    

| 问题 | 描述 | 成因 | 方案 |
|:--------:|--------|--------| --- |
| 梯度消失 | 梯度趋近于 0 | 激活函数变化平缓 | `Relu`、`BN`、`LSTM`/`GRU` |
| 梯度爆炸 | 梯度趋近于无穷大<br>伪梯度爆炸 |  | 梯度截断、`BN`、平滑的激活函数<br>调小学习率、增大批次、warmup |
